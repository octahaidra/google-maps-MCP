...
function getApiKey(): string {
    ...
    if (!apiKey) {
      console.error("GOOGLE_MAPS_API_KEY environment variable is not set in .env file");
      process.exit(1);
    }
    console.log("API Key loaded:", apiKey.substring(0, 5) + "..."); // Only log first 5 chars for security
    return apiKey;
  }
...
export async function handleElevation(locations: Array<{ latitude: number; longitude: number }>) {
